...
const finishLedgerDeviceInfo = ( msg ) => {
    if ( LOG_LEDGER_MESSAGE ) {
        console.log( 'getLedgerDeviceInfo : ', JSON.stringify( msg ) );
    }
    return msg;
}
...
const getLedgerDeviceInfo = ( callback ) => {
    ...
    if ( !supported ) {
        callback( finishLedgerDeviceInfo( { enabled: false, message: 'Your computer does not support the ledger device.' } ) );
        return;
    }
